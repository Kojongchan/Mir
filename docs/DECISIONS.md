...
| D14 | **3D 모듈을 용도별로 분리**: `통합모델(3D)` · `공정관리(4D)` · `간섭검토` 가 각각 **자기 용도(purpose)의 모델 세트만** 본다 (`models.purpose` = integrated/4d/clash, `0016_model_purpose.sql`). 4D/간섭은 통합모델과 별개로 모델을 다시 업로드해 활용. **S33** | 사용자 결정: 4D 는 공정표 매핑으로 날짜별 객체 표시가 달라져 간섭검토를 함께 보기 어렵다. 모듈별 모델을 분리해야 각 기능을 방해 없이 쓸 수 있음. 통합모델 3D 는 이슈 생성 + 이슈 핀 토글의 마스터 뷰 |
