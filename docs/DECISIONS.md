...
| D22 | **프로젝트 관리자(admin 역할)는 멤버·역할·계정생성까지, 프로젝트 설정은 시스템 관리자 전용**. 프로젝트 관리자는 자기 프로젝트의 구성원 화면(`/project/:id/members`, `ProjectMembers`)에서 기존 사용자 배정·역할 변경·해제, **신규 로그인 계정 생성**(자기 프로젝트에 자동 배정), 멤버의 비번/아이디 변경까지 가능(시스템 관리자 멤버·계정삭제·프로젝트설정 제외). 서버 `api/admin.ts` 가 호출자=시스템관리자 OR 해당 프로젝트 admin 으로 인가하고 대상은 그 프로젝트 멤버로 스코프. **프로젝트 설정(이름·ACC 고정)·프로젝트 생성/삭제·계정 삭제·시스템 관리자**는 시스템 관리자 전용(`0025` 로 projects update 를 is_admin 으로 환원). 멤버 화면 접근·좌측 메뉴는 `canManage`(useProjectRole). **S48** | 현장 관리자가 자기 팀(계정·역할)을 직접 꾸리되 시스템 설정은 못 건드리게. RLS(0023, project_members=프로젝트 관리자 허용) 위에 서버 스코프·UI 게이팅을 얹어 다층 강제 |
