...
### ★★★ 단일 해결 플랫폼 원칙 (최상위 — 사용자 확정)
> **MIR SMART 안에서 다 끝난다. 원본 저작툴(Navisworks·Revit·Solibri)로 돌아갈 필요가 없어야 한다.**
> 모든 검토·협의·마크업·해소·보고를 **웹에서 완결**한다. 이것이 근본 차별점.
- 설계 함의: 워크플로우가 **플랫폼을 떠나야 완성되는 구조를 만들지 않는다**(예: "BCF로 내보내 NW에서 열람" =
  목표 아님). 협의·의사결정·이력이 전부 플랫폼 안에 남는다.
- 외부 표준(BCF·IFC 등)은 **"가져오기(레거시 흡수·온보딩)"만 선택적** 지원. **"내보내 돌아가기"는 비목표.**
  출력이 필요하면 **웹 네이티브 뷰 + 회사 양식 PDF/엑셀**로(다시 저작툴 여는 용도 아님).
...
1. **간섭검토** (플랫폼 내 완결): 검출→**3D 마크업/뷰포인트 첨부**→**담당 배정(관계사/역할)**→**코멘트 스레드·
   멘션**→상태 추적(open→해소→검증)→**회사 양식 보고서**(스냅샷·요약표→PDF/엑셀, S38). 규칙셋 저장·정기 재검사·
   상태 승계(S40). 뷰어중심 UI. **BCF는 레거시 "가져오기"만 선택적**(내보내 NW로 돌아가기 X).
