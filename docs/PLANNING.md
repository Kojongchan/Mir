...
### 권장 진행 순서 (메뉴 고도화)
가치·완성도 낮은 순 + 인프라 재사용 고려: **게시판(→회의록 R2) → 협업·이슈(대시보드+RFI/Punch) →
자료관리(ISO19650) → 공사일보(모바일) → 물량/기성(원가·EVM) → 통합모델(V2/V3) → 공정현황(EVM) →
구성원·권한(역할·관계사) → 사업개요(실시간) → 4D·도면·하도급·간섭 보강.**
> 각 메뉴 고도화의 상세 착수 문구는 `docs/handoff/R-series.md`(기능)·U-시리즈 핸드오프(디자인) 재사용.
> 신규 대형 기능(Track C·R-AI)은 이 고도화 사이클 이후 또는 예산/데이터 결정 시.
