...
- **사용자 탭**: 아이디(한글 가능)·표시이름·비밀번호로 생성, 관리자 지정/해제,
  비밀번호 변경, 삭제. 생성 시 `username` 메타데이터를 직접 넣으므로 비-ASCII 보정
  SQL이 **불필요**합니다.
- **멤버 배정 탭**: 프로젝트별로 사용자 추가/역할 변경(viewer·editor·admin)/해제.
- 📌 사용자 생성/삭제/비번변경은 `/api/admin` 서버리스 함수가 필요 → **배포 환경
  (Vercel)** 또는 로컬 `vercel dev`에서 동작. 일반 `npm run dev`(vite)에서는 사용자
