...
네 번째 진단: 선택/로드12, 후보180, resident18(이전 위치6), 사용146MiB.
레거시 manifest에서 크기 없으면16MiB 예약해 첫12개 선택. 실제 파일이 작아도 select는
카메라가 바뀔 때만 실행되어 후보를 보충하지 않음. 작은 모델로 수정 전 실패 재현.
- TileStream이 전체 후보 목록을 보존하고 각 로드 완료 후 실측 크기로 다시 계획.
- 기존192MiB/24개/동시2 제한 유지. 필요한 경우 기존 LRU 규칙으로 이전 구간 해제.
- 수정 전 회귀:40예산/16추정/8실측/4후보에서2개로 정지; 수정 후4개 로드.
- 검증:22개 회귀, 타입/빌드. 실제 배포 NWD/XKT 형상 검증 아님.
미해결: 구간 단위 완전성, 화면 범위 우선순위, 전체현장 LOD, 렌더 성능.
사용자가 진단 JSON만 제공했으며 실제 XKT(sample ZIP)는 아직 없으므로 뾰족한 면 재현 불가.
다음: 실제 XKT 확보 후 단독 렌더 비교. 이 수정도 전체 구조물 표시 해결로 보고하지 말 것.


## 2026-09-15 — 실제 XKT 재현자료 확보 경로

사용자 세 번째 진단: 구조물 위치는 일치하기 시작했으나 일부 끊김/뾰족한 면, 이동 평균163ms.
후보232개 중20개 선택, 실제 resident는24개(이전 구간4개 포함). 180MiB는 GPU예산 아님.
- 변환 코드: 동일 셀도 삼각형 상한으로 여러 청크 분할. 현재 개별 청크 선택은 공간/부재
  완전성을 보장하지 않음. 일부 미표시 원인과 원본/변환/렌더 형상 오류를 구분해야 한다.
- 로딩 상한을 올리지 않음. 현재 일부 표시 상태를 후보/로드 개수로 명시.
- 검증 구간 저장: 클릭 위치(없으면 camera.look)와 가장 가까운 로드된 구조물 청크1개를
  기존 URL로 다시 읽어 sample.xkt + sample.json ZIP 저장. 원본 URN/서명URL/토큰은 제외.
  32MiB 읽기 상한,60초 타임아웃,모델 교체 시 취소. 새 변환/자동 업로드 없음.
- 검증: 기존19개 + 다운로드 상한/취소2개=21개, 타입 검사, 빌드.
실제 형상/회전 문제 수정 완료 아님. 다음 입력: 사용자가 문제 부재를 클릭하고 검증 ZIP 제공.
다음: 같은 청크를 단독 렌더/VBO와 DTX 비교, 형상/면 방향/정밀도 확인, 주변 청크 완전성과 분리.
근본 과제: 셀 단위 완전성 메타데이터+오차 기반 LOD 및 공간 분할 인스턴싱. 현재 캐시만으로
전체 현장 누락 없음/부드러운 회전 보장 불가. 재변환은 기본 비활성 유지.
인수인계: 더 많은 화면 옵션이나 임의 예산 변경보다 실제 XKT 재현을 우선할 것.


## 2026-09-15 — 로드 구간과 카메라 불일치 수정

사용자 두 번째 JSON: 구조물 12개 로드 성공, AABB는 X3977~4179/Z2522~2676.
카메라는 X5357/Z3880: 로드한 구간과 약 1.8km 떨어져 있음. eye-look은 약0.226,
near=0.5보다 짧음. 파일 로드 실패가 아니라 화면 위치와 작업 세트 불일치가 확인됨.
- 전회 수동 구간 고정 수정: 단순 orbit은 유지하지만 look 이동/줌 변화가 여유 범위를
  넘으면 정지 후 지역 자동 재선택. 예산과 동시성 제한은 유지.
- 최초 구조물 작업 세트 로드 뒤 실제 구조물 AABB로 한 번 맞춤. 구조물 맞춤 버튼도 추가.
- 지형에 가린 지하 부재 확인용 지형 숨김/표시(베이스만) 추가.
- 반복 휠 확대 시 시선 중심 거리가 max(2, near*4)보다 짧아지는 경로 제한.
- SDK 삼각형 getter=0을 실제 0삼각형으로 단정하지 않고 집계 미지원/null로 표시.
검증: 지역 이동/회전 유지/반복 확대 회귀 포함 19개 테스트, 타입 검사, 빌드.
실제 XKT 렌더링/배포 시각 검증은 아직 수행하지 못함. 전체현장 LOD 및 회전 병목 미완료.
다음: 현재 카메라와 로드된 구조물의 위치 일치 확인 후 필요 시 해당 XKT로 시각 재현.
인수인계: 전체 모델 해결로 보고하지 않음. 화면 바깥 타일 로딩과 미표시를 구분할 것.


## 2026-09-15 — 최초 로딩 빈 구간 고정 회귀 수정

사용자 JSON: base0만 존재, 구조물 후보/선택/로드 0개. 전회 수동 구간 고정 후 발견.
- 최초 frameOnce의 카메라 flyTo가 비동기인데 직후 카메라 위치로 구간을 선택하고
  더는 자동 갱신하지 않는 경로 수정. 최초 선택은 목표 AABB 중심/범위를 사용한다.
- 초기 목표 구간이 비었으면 가장 가까운 구조물 구간으로 후보를 확보한다. 같은 예산 유지.
  수동 구간 선택은 빈 구간을 그대로 보고하며, 최초 폴백처럼 다른 지역을 임의 선택하지 않는다.
- 유효한 지형/focus AABB가 없을 때 타일 범위를 초기 프레이밍에 사용.
- 후보 0개를 정상 고정 완료로 안내하지 않음.
검증: 회귀 테스트 17개, 타입 검사, 빌드. 실제 배포 모델 재확인은 아직 필요.
다음: 자동으로 구조물 후보가 잡히는지 확인 후 기존 회전 병목 조사 계속.
인수인계: 제공된 38.5ms 이동 프레임 수치는 지형만 로드한 상태이며 구조물 성능으로 해석 금지.


## 2026-09-15 — 표시 구간 고정 및 회전 진단 (feature/3d-streaming-stability)

사용자 실측: 시점에 따라 부재가 나타났다 사라지고, 구조물 로드 후 회전 급감.
이전 제한(24타일/192MiB)은 전체 모델 누락과 렌더 부하를 해결하지 못했다.

- 타일 경로의 카메라 정지 후 자동 재선택 제거. 최초 구간을 유지하며, 다른 구간은
  `현재 위치 불러오기` 버튼으로 명시적으로 교체. 전체 현장 LOD의 대체가 아닌 임시 검토 모드.
  처음부터 예산으로 제외된 타일은 여전히 미표시이며, SDK 컬링/원본 면 누락은 별도 검증 필요.
- FastNav 대신 NavigationQuality: 드래그 중 해상도 0.5 유지, 해제/마지막 이동 후
  350ms에 복원. SDK의 이전 프레임 deltaTime 차감으로 느린 이동 프레임에서
  조기 복원될 수 있는 경로 제거. 이 효과가 실제 병목에서 차지하는 비중은 미측정.
- 연속 휠 입력은 표면 pick 결과를 재사용(200ms 중단 시 새 선택). 클릭/더블클릭 선택 유지.
- 최근 이동 프레임 간격, CPU 렌더 제출시간(GPU 시간이 아님), longtask, 모델별
  SDK 추정 삼각형/엔티티/AABB와 카메라를 로컬 JSON으로 저장. API 키/URL 전송 없음.
- 검증: 스트리밍/내비 회귀 테스트 15개, 타입 검사, 프로덕션 빌드. 실제 모델 런타임 미검증.

다음: 이 브랜치 테스트 배포에서 모델을 열어 10초 회전한 뒤 `진단 저장`.
고정한 구간도 사라지는지 확인하고 필요 시 해당 XKT와 manifest로 컬링/면 방향을 재현.
미해결: 전체 커버리지와 화면 오차 기반 계층 LOD, 구조물 기하량/드로콜 부하,
현재 예산 제외 구간, 실제 GPU 시간. 예산을 올리거나 전체를 일괄 로드하지 말 것.
인수인계: 누락·회전 문제 해결 완료로 보고하지 말고, 실측과 대표 타일로 다음 수정 결정.


## 2026-09-11 — 3D 테스트 안정화 (feature/3d-streaming-stability)

사용자 요청: 기존 테스트 브랜치에서 개발을 이어가되 추가 비용을 피한다.
이번 범위는 누락/로딩/변환 안전장치이며 엔진 교체·CDE 저장소 이전은 하지 않았다.

- 카메라 이동 중 타일 숨김 제거. 동시 2개, 최대 24타일/압축크기 기준 192MiB 예산.
  이 값은 GPU 메모리 상한이 아니다. 베이스 지형은 별도이며 전체 현장 LOD는 아직 미구현.
- 네트워크 취소, 로딩 타임아웃, 1회 지연 재시도, 오래된 타일 해제, 모델 교체/언마운트 정리.
- 넓은 뷰에서 원본을 모두 버리던 overviewOnly 경로 제거. 전체 완성으로 표기하지 않고
  현재 예산 내 주변 구간 보기임을 안내. 기존 거친 lod1은 다시 활성화하지 않음.
- legacy manifest.inst는 부재를 조용히 누락하거나 일괄 로드하지 않고 명시적으로 호환 오류 표시.
  다음 단계: 반복 형상은 보존하면서 인스턴스 배치를 공간 분할하는 변환 설계 필요.
- API 새 변환은 ENABLE_MODEL_CONVERSION=true일 때만 허용(기본 꺼짐). 기존 캐시는 열람 가능.
  활성화하더라도 APS SVF 새 작업 요청 코드는 제거됨. 기존 SVF가 없으면 종료.
- 변환 브랜치 GH_REF(또는 Vercel 배포 브랜치) 필수. 웹 요청은 tiles=1/tile_m=200/
  tile_cap=300000/instance=0 고정. 새 매니페스트에 빌드 ref/commit/옵션/타일 크기 기록.
- 청크 실패/공간정보 누락 시 게시 중단. XKT 청크는 실행별 디렉터리에 저장해 이전 정상
  매니페스트의 파일을 덮지 않음. 기존 캐시 선삭제 제거. 이전 실행 파일 자동 정리는 미구현.
- 저장소 경고를 강제로 우회하는 UI 제거. 사용량 조회 실패 시 변환 중단.
  수동 Actions 실행이나 기존 서비스 사용량까지 무료로 보장하는 기능은 아님.

검증: npm run test:streaming(13개), npm run typecheck, API 별도 타입 검사,
node --check scripts/convert4d.mjs, npm run build. 실제 NWD/XKT/R2/배포 FPS 검증은 미실행.
이번 작업에서 workflow_dispatch, APS 변환, R2 업로드/삭제, 운영 병합은 실행하지 않음.

다음: 기존 캐시가 inst 사용 빌드인지 확인 → 실제 모델로 회전/줌/재방문/장시간 측정.
아직 남음: 화면 오차 기반 LOD, 메인 스레드 XKT 파싱, URL 자동 갱신,
URN 40자 캐시 키 교체, 모델별 API 권한 검증, 변환 작업 잠금/용량 예약/이전 파일 정리.
변환을 다시 켜기 전에 용량 예약과 권한·캐시 키 보강이 필요하다.

인수인계: 문서의 ‘전체 채움/인스턴스 항상 표시’는 현재 구현 설명으로 사용하지 말 것.

---
## 🚀 인스턴싱 + 뷰어 즉시 개선 (2026-09-09)
> branch `claude/3d-view-testing-hlrugl`. 뷰어 수정 2건 배포됨. 인스턴싱 변환 빌드 대기.

### 뷰어 즉시 수정(재변환 없음, 배포됨)
- **전경 누락**: 타일 선택을 look(궤도중심)→**시선 전방(eye)+근접 우선**으로. 조감뷰에서 카메라
  앞 전경이 빠지던 문제 해소. 동시 다운로드 6→8. (commit 938b298)
- **회전 중 끊김**: 이동/회전 중 새 로딩 **보류(settle-gated)**, 0.28초 정지 후에만 로드 → 회전 시
  트래픽 몰림·끊김 제거. (commit 71a5f30)

### 인스턴싱 측정(진단, 변환 없이 2분)
- flatten 4.12억 → 고유 1.80억 = **반복배수 2.29배**. 전체 노드 49만 중 **23.7만(절반)이 6,476종
  반복 부재**(교각/거더/침목류, 각 260~300회). convert2xkt 는 accessor 해시로 인스턴싱 지원 확인.
- 레이어 분리 실측: **임계 refs>=16 → 항상로드 ~43MB(반복부재 92%=21.8만 인스턴스), 타일 4.12억→
  1.93억(2.1배 경량)**. 반복 많은 부재만 담으면 싸게 대부분 포섭.

### 인스턴싱 구현(XKT_INSTANCE=1, 기본 refs>=16 = XKT_INST_MIN)
- `mergeGlb`: 반복+무텍스처 노드를 타일에서 빼 **인스턴스 레이어**(로컬 형상 1벌+인스턴스별 행렬)로
  방출. `makeStream.addInstance`(geomKey별 accessor 1벌 공유 → convert2xkt dedupe). geomId로 묶어
  같은 청크(dedupe 보존), 고유 지오메트리 90MB마다 flush. 색은 인스턴스별 material 유지.
- `convert4d`: kind 'inst' → `manifest.inst` + 트래픽 로그. `api`: `instUrls` presign.
- `ThreeDTest.mountTiles`: instUrls **항상 로드·항상 표시**(지형 베이스처럼). 반복 구조물이 어디서나
  즉시(스트리밍 대기 없음) + 남은 고유 타일은 2배 경량.
- 원본 형상 100% 유지(감량/깍두기 아님). 기대: 최초 ~253→~296MB(+43MB), 구조물 절반 즉시, 타일 절반.

---
## 🚀 뷰 스트리밍 3차 — 지형베이스 분리 + 영속 타일 빌드 완료 (2026-09-09)
> run 34296075984(sha 729d312, tiles=1, 셀200m). 앱 런타임 검증 대기(사용자).

### 이번 빌드 (구조 변경)
- **지형(115만 삼각형, 항공드레이프)** → 항상 로드되는 **베이스 `b*.xkt`**(줌아웃해도 바닥 유지).
- **구조물(3.61억 삼각형)** → **영속 공간 타일**(보는 곳만 스트리밍, 이동해도 언로드 안 함=재방문 즉시).
  LRU 상한 44타일. lod1 = 구조물-only 거친 개요.
- 뷰어 mountTiles: 베이스 항상 표시, lod1은 줌아웃(overviewOnly)일 때만. 회전/미세이동 재계산 생략.

### 실측 결과 (convert 로그)
- 구조물 타일 **341개** · 지형베이스 **1청크(115만 삼각형)** · 셀 200m.
- **최초 로드 = 지형베이스 243.3MB(항상) + 개요 9.8MB = ~253MB.** 구조물 타일 평균 **24.9MB/개**(보는 곳만).
- ⚠ **최초 253MB는 목표(<100MB) 미달.** 원인 = 지형 항공사진 텍스처(1024²×약100장 ≈ 228MB, 베이스 대부분).
  지형은 항상 로드라 이게 매 뷰 고정비용. → **다음: 항공사진 텍스처 감량(1024→512 ≈ 4배↓, 베이스 ~60MB)**
  이 최우선. 셀당 텍스처 통합/재사용도 검토.

### 검증 요청 (사용자, 하드새로고침 후)
(a) 바닥(지형)이 항상 보여 빈 구멍 없는지  (b) 줌인 시 구조물이 원본으로 채워지는지
(c) 봤던 곳 되돌아가면 재로딩 없이 즉시인지  (d) 회전 시 재로딩 없는지.

---
## 🚀 뷰 스트리밍 2차 — 작은 타일(423개·평균23MB) 빌드 완료 (2026-09-09)
> run 34290666112(sha 11ddcb2, tiles=1, 셀200m·타일상한120만). 앱 런타임 검증 대기(사용자).

### 배경 (실측으로 방향 확정)
- 실측: 지형 115만 · **구조물 3.61억 삼각형(원본 8.7GB)**. 브라우저 원본 통째로드 불가 확정.
- 사용자 선택(AskUserQuestion): **원본 우선 뷰 스트리밍**(이동 시 로딩 대기 감수).

### 이번 빌드
- 타일 셀 500→**200m**, 타일당 상한 **120만 삼각형**(XKT_TILE_CAP) → **타일 423개, 평균 ~23MB**
  (이전 ~100MB/타일 → 개별 로드 몇 초). 원본 형상 그대로. lod1 개요 포함. 총 9.6GB(1회 업로드).
- 뷰어 mountTiles 튜닝: 개요전환 임계 0.30(가까이서만 스트리밍), 로드반경 min(camDist*1.2,1400)/
  350~, 동시 5, MAX_LOADED 32, **회전·미세이동엔 재계산 생략**(look 이동·줌 변화 클 때만).
- 복구: 8.7GB 원본단일로드(run 34172823849)로 앱이 멈춰 → 비타일 로드가능 빌드(run 34186970432)로
  복구 후 이 타일빌드로 덮음.

### 검증/사용법
- ⚠ 앱 런타임 미검증(헤드리스 불가). 사용자 하드새로고침(최신 배포) 후: **줌아웃=개요(거친 회색)**,
  **줌인=그 구역 원본 구조물이 몇 초 내 채워짐**, 회전 시 재로딩 없음. 처음 열면 전체맞춤=줌아웃이라
  개요만 보임 → **보고싶은 곳으로 줌인**해야 원본 스트리밍 시작.
- 남은 튜닝: 줌인 광역뷰에서 최대 32타일(~15s) 로드. 개요가 회색(무텍스처)이라 줌아웃 볼품 없음
  → 개요를 감량-텍스처(항공드레이프)로 교체하면 개선(후속). 타일당 여전히 크면 상한 더 낮춤.
...
### 문제(사용자 지적 정확)
- 지형 항공사진 타일들의 **경계선(seam)** 이 보이고 산·도로가 부자연스럽게 붙음 = 이미지가 제대로 안 들어옴.
- **근본 원인**: `svf-utils` 재질 파서가 텍스처 변환에서 **scale 만 읽고 offset 을 버림**
  (`materials.js:134 // TODO: parse texture transforms aside from scale`). 지형 V UV 가 `[1,2]` 로 정수만큼
  밀려 들어오는데, 여기에 flip(1-v) 을 그대로 걸면 `[-1,0]` 으로 튀어 이미지가 **접히고 경계선** 발생.

### 해결(커밋 fe71e40)
- `mergeGlb.mjs` 텍스처 UV 계산: 프래그먼트별 **정수 offset(floor(min)) 을 빼 UV 를 [0,1] 로 정규화한 뒤**
  flip 적용. CLAMP sampler(wrapS/T 33071)는 이제 [0,1] 이라 올바름(가장자리 bleed 방지).

### 검증(서버측 완료 · run 33821261072)
- convert 로그 `[tex] frag → 정규화 UV` 실측: 이전 V[1.49~2.00]·[1.00~1.52] → **수정 후 전부 [0,1]**
  (#1 V0.00~0.51, #2 V0.48~1.00, #3 V0.52~1.00, #5 V0.00~0.54 …) — **인접 타일 연속 매핑 확인**.
- LOD1 오블리크 렌더 88.9% 채움, c0 텍스처 렌더 KTX2 정상 디코드(크래시 없음).
- **앱에서 새로고침하면 경계선 사라짐**(R2 캐시 갱신됨).

### 부가 — 트래픽(전산실) 계측 로그 추가
- `convert4d.mjs` onChunk 가 업로드 XKT **바이트(MB)** 누계를 로그로 남기도록 추가:
  `다운로드 용량(트래픽): 상세 X MB + 개요 Y MB = 최초 Z MB · nav(회전시) W MB`.
  이번 run 이전 커밋이라 **다음 변환부터** 실측 MB 가 로그에 찍힘.

### ⚠ 현 파이프라인 상태 / 미해결
- 현재 R2 캐시 = **상세 1청크(c0, 4.8M 삼각형) + LOD1(1.1M)**. 상세 c0 는 clusterUV/clusterAbs
  **감량본**(XKT_CELL 1.5m). nav 0청크. 원본 3.62억 대비 ~75배 경량 → 빠름·트래픽 급감.
- **미해결(사용자 반대 방향 주의)**: 사용자는 **형상 감량을 최종 해법으로 거부**(Autodesk DOCS 식
  1회 업로드 처리 후 뷰 종속 다중해상도 스트리밍 = 원본 보존 요구). 구조물 각짐도 이 스트리밍으로
  해결해야 함(감량 아님). 이 대규모 파이프라인 재구축은 **미착수** — 착수 전 사용자 합의 필요.
- rotation 여전히 다소 느림(엔티티 오버헤드 추정, 미검증). 실제 트래픽 MB 다음 변환에서 계측 예정.
...
- 증상: 지형 항공사진은 뜨나 **회전이 극도로 느리고(멈췄다 튐)**, 회전 중 지형이 **흰색**으로 번쩍임.
- 원인: ① 3.62억 삼각형을 움직이는 매 프레임 전부 그림(FastNav 화질저하만으론 지오메트리 부하 그대로).
  ② FastNav `hideColorTexture:true` → 회전 중 텍스처 off → baseColor(흰색) 노출.
- 수정: **모션 기반 LOD**(setupSwap 재작성). 카메라 이동 중엔 상세 73청크 숨기고 LOD1(~110만 삼각형·
  단색 회녹색)만 그림 → 부드러운 회전. 멈추면 220ms 뒤 상세(항공사진) 복원. 정지=항상 상세.
  FastNav `hideColorTexture:false`(회전 중 LOD1 단색만 보이므로 무관, 흰색 번쩍임 제거).
- 트레이드오프: 회전 멈춘 순간 상세 3.62억 그리는 1회 settle(~1-2s). 회전 자체는 부드러움.
- 검토 포인트: LOD1 은 4m 격자 개요(무텍스처 회녹색). 회전 중엔 개요만 보이고 멈추면 사진 복원.
...
### ★★★ 결정적 버그 3: gammaOutput(sRGB) — 지형이 검게 (수정 완료)
- 실모델 c0.xkt 렌더에서 지형이 사진 대신 **검게**. 원인: 텍스처는 sRGB 인코딩인데
  `viewer.scene.gammaOutput` 이 꺼져 있어 xeokit 가 선형공간 그대로 출력 → 중간톤(항공사진의
  회색·갈색·녹색)이 거의 검정으로 뭉갬(포화 원색만 살아남음). → `gammaOutput=true`(ThreeDTest·
  renderDiag). 로컬 4변형 실측으로 gamma 가 결정 요인 확인, 실모델 렌더로 최종 확인(run 33167368403,
  cef0b11): 중간톤(흙·식생) + 표면 디테일 정상 표시(이전 흰색/검정 해소).

### 결론 — 지형 항공사진 파이프라인 완성 (버그 4개 모두 수정)
1. 재질 `_svf_tex_mod` 접미사 → 원본 항공사진 매칭 (텍스처 못 찾던 문제)
2. `KTX2TextureTranscoder` 생성자 시그니처(단일 옵션객체) — 텍스처 로딩 통째 크래시
3. 텍스처 **POT 리사이즈**(sharp) — 블록압축 GPU 요건(4의 배수) → 흰색 해소
4. **gammaOutput=true** — sRGB 텍스처 검정 뭉갬 해소
   \+ self-host basis 트랜스코더(/basis/), 자가진단 배지, 렌더 진단 견고화.
- ③④는 **실모델 데이터 검증**에서만 드러남(합성 테스트는 우연히 통과). 각 버그 재현→수정→렌더 확인.
- R2 최신(POT 텍스처, 73청크·3.62억 삼각형·텍스처 프래그 15589). Vercel 이 브랜치 push 마다 빌드.
- 남은 판단: 실제 앱(실 GPU·전체 모델)에서 밝기/가독성 확인 — 필요 시 ambient light 보강.
...
### 11차 — 스무스 LOD 검증 완료 후 재활성 (커밋 d29e83b)
- **render_test 재변환 성공**(run 31684996258): 스무스 lod1(Z 실제값·4m) 캐시 반영 + CI 헤드리스
  렌더로 실모델 LOD1 확인 → ASCII 88.9% 채움·매끈한 지형 실루엣·구멍 없음. 합성 before/after(주황
  계단 vs 파랑 매끈)와 합쳐 **계단 수정 검증 완료**(사용자에 이미지 전송).
- **LOD 재활성**(`mountXkt`): 시점거리 기준, 디테일 기본 / 개요는 전체맞춤(~diag) 이상 줌아웃만
  (FAR=0.65·NEAR=0.45·diag). lod1 숨긴 채 먼저 로드, 상세 보이게 순차. 줌인=풀디테일, 줌아웃=스무스
  개요 → 렉·형상 둘 다 해결.
- 검증도구: 로컬 헤드리스 렌더(renderDiag+PW_CHROME/PW_SAVE/PW_OBLIQUE, scripts/testLod.mjs).
  URN 확보로 내가 직접 재변환 dispatch 가능(run_workflow). ⚠ SessionStart 훅이 playwright-core
  프루닝하므로 로컬 렌더 재실행 시 재설치 필요.
- 남은 것: ④지형 텍스처=SVF 에 UV 없음(외부 오쏘포토 드레이핑 별도), ③형상 정밀도(2cm weld 조정).

### 10차(자율 야간) — LOD 계단 원인 규명·수정 검증 + 로컬 렌더 도구 확보
- **전체모델 렌더 성공 확인**(사용자 스샷, 엔티티 15.1만). 사용자 지적: 먼 줌에서 뜬 개요(LOD1)가
  **계단(논밭)+구멍**으로 형상이 깨져 보임 → "제대로 보여야 함".
- **핵심 도구**: CI 로그에서 **모델 URN 확보** → `actions_run_trigger` 로 **내가 직접 재변환 dispatch**
  가능(사용자 반복 클릭 불필요). `renderDiag.mjs` 에 `PW_CHROME/PW_SAVE/PW_OBLIQUE` 추가 →
  **로컬 헤드리스 렌더 검증** 가능(이 환경 Chromium).
- **계단 원인 규명(로컬 검증 완료)**: LOD1 이 대표점의 **Z(높이)까지 격자 스냅** → 8m 계단. 합성
  지형으로 재현(zsnap=계단 vs zfixed=매끈, before/after 렌더로 확인·사용자에 전송). **수정**: Z 는
  실제값 유지(XY 만 격자), 셀 8m→4m. 커밋 7d06eef.
- **프런트 잠정 안전화**: LOD 스왑 비활성(`LOD_ENABLED=false`) — 항상 풀디테일. 상세 청크를
  '보이게' 순차 로드(점진적 형상). 거친 개요 안 뜸(커밋 a2699ad). ⇒ **내일 사용자는 풀디테일만 봄.**
- **진행 중**: `render_test=1` 재변환(내가 dispatch) → 실모델 LOD1(구조물 포함) CI 렌더(base64 PNG)
  확인 예정. 매끈하면 LOD 재활성(높은 임계=먼 줌만) + 스무스 lod1 캐시 적용. 구조물이 뭉개지면 LOD off 유지.
- **텍스처(④)**: SVF 지오메트리에 **UV 없음 확정**(`[tex] probe … reading 'uvs'`) → 원본에 지형
  이미지 없음. 정사영상 드레이핑은 별도(사용자 정사영상+좌표 필요).
- **형상 정밀도(③)**: 2cm weld(XKT_WELD_Q=50) 후보 — 필요 시 1cm 로. 실모델 확인 후 판단.

### 9차 — 우리 xeokit 확장: LOD 스트리밍 + 텍스처 감지 (사용자 피드백 5종 대응)
- 8차에서 통합모델(3.6억 삼각형) **렌더 성공**(엔티티 15.1만) 확인. 사용자 피드백: ①모델·지형 뜸
  ②렉 심함 ③형상 100% 아님 ④지형 이미지 맵핑 누락 ⑤xeokit 상위버전 필요.
- **렉(②) 1차**: `FastNavPlugin`(이동 중 SAO/PBR/텍스처/엣지 off·해상도 0.5x) + `saoEnabled:false`.
  프런트만(재변환 불필요, 커밋 f84e55b).
- **렉(②) 근본 = LOD**(커밋 435db4c): `mergeGlb` 스트리밍 중 전역 8m 격자로 **개요(LOD1)** 누적
  → `lod1.xkt`. 뷰어(`mountXkt`)는 **개요 먼저 표시 → 상세 73청크 숨김 백그라운드 로드 →
  카메라 거리로 LOD 전환**(줌아웃=개요만, 줌인=상세+프러스텀컬링). manifest `{xktFiles, lod1}`,
  api 가 lod1 presign.
- **텍스처(④)**: SVF 에 실제 이미지/UV 유무를 `[tex]` 진단 로그로 확인(재질 키·UV 길이, 처음 5개).
  지형이 초록 단색이라 없을 수도 → 있으면 추출·맵핑, 없으면 외부 오쏘포토(별도). 되는 파이프라인
  안 깨고 확인하는 안전 접근.
- **⚠ 다음(사용자)**: **재변환 1회** → 내가 로그로 확인: `LOD1(개요) 방출 … 삼각형 N`(LOD 생성),
  `[tex] frag# matKeys=… uvLen=…`(텍스처 유무). 이후 LOD 임계값 튜닝 + (텍스처 있으면) 추출 착수.
  형상 정밀도(③)는 `XKT_WELD_Q` 상향으로 조정 예정.

### 8차 — XKT 스트리밍 변환 **성공**(전체 모델, 감량 0) + 브라우저 로드 튜닝
- **변환 성공 확정**(run 31651820371, sha 98b1f2f, 36.5분): `XKT 스트리밍 완료: 청크 73개 · 정점
  215,975,070 · 삼각형 361,804,806` → R2 `<keyBase>/xkt/` 업로드. **OOM·shard·과감량 전부 없음.**
  convert2xkt 가 73개 청크(멀티메시·uint32 인덱스) 전부 소화 — 마지막 미검증점 통과.
- 프런트 30분 폴링 타임아웃은 무관(CI 37분 완료, 캐시엔 XKT 존재) → **다시 열면 캐시 히트**.
- 브라우저 로드 튜닝(2.16억 정점 대비): `ThreeDTest` Viewer **dtxEnabled:true**(데이터텍스처 —
  대용량 메시 GPU 메모리 급감, XKT 는 순수 메시라 무관; DWG 선형은 별개 후속) + XKT **순차 로드**
  (73개 병렬 파싱 메모리 스파이크 방지, 진행률 표시).
- **⚠ 남은 관문 = 브라우저가 3.6억 삼각형을 감당하는지**(극한 규모). dtx 로 최선. 부족하면 선택적
  로드(뷰 기반)나 거대모델은 APS Viewer(통합모델 3D). 사용자 재오픈 결과로 판단.

### 7차(구현) — XKT **스트리밍 청크**로 OOM 해결 (메모리 상한 고정)
- **6차 결과(run 31578007105, sha 6d0481c)**: XKT 재변환 실제 실행 → **OOM(exit 143)**. 로그 확정:
  원본 삼각형 **3억6180만**, 단일그룹 최대 **정점 7424만·삼각형 1억3043만**(코리더 표면), 지형
  정점 5306만·삼각형 9639만. `perGroupDir` 는 그룹 Map 에 **전부 쌓은 뒤** 내보내 12GB 초과.
- **수정(스트리밍)**: `mergeGlb` `opts.xktStreamDir` + `opts.onChunk` — 그룹 Map 안 쓰고 프래그먼트를
  '청크' GLB 에 흘려담아 **CAP(기본 300만 삼각형)마다 flush → onChunk 가 즉시 convert2xkt→XKT→R2
  업로드→GLB/XKT 삭제**. 메모리·디스크 상한이 **한 청크(~150-200MB)** 로 고정 → **OOM 원천 불가**.
  프래그먼트=자기 색·노드(감량 없음, 원본 정밀도). `_DBID` 정점속성 제거(convert2xkt 위험) →
  node.name 에 dbid.
- **로컬 검증**: 청크 GLB 바이트 구조(헤더·JSON/BIN 청크·오프셋 4정렬·bufferView 범위) 정합 확인
  (scratchpad/glbtest). convert2xkt·xeokit 로드는 CI/브라우저에서만 검증 가능.
- **⚠ 내일 할 일(사용자)**: 앱에서 **"재변환" 1회** → 제가 CI 로그(`청크 N → cN.xkt … 업로드`,
  `XKT 업로드 완료: N 청크`) 직접 확인. convert2xkt 가 청크 GLB(멀티메시·uint32 인덱스)를 잘 먹는지가
  마지막 미검증점. 실패 시 로그 보고 이어서 수정. (URN 이 CI 에 마스킹돼 내가 직접 dispatch 는 불가.)
- 폴백: 계속 말썽이면 거대 통합모델은 **통합모델(3D) 메뉴=APS Viewer**(이미 스트리밍 렌더) 사용.
...
- **로드 성공·shard 없음 확정**(엔티티 101, span 7613×251×6571). 남은 문제 2개:
  - 지형 가장자리 너덜/검은 삼각형 홀 → **원인**: 클러스터 대표점이 '첫 정점'이라 인접 패치가
    서로 다른 점으로 병합돼 이음새 크랙. **수정**: 대표점을 **월드 격자점**(`round(p*q)/q`)으로 —
    같은 셀은 어느 패치서든 동일 좌표 → 봉합 seamless(홀 제거). `weld`/`weldPos` 둘 다.
  - 전체가 납작한 흰색(음영 없음) → **원인**: 클러스터 후에도 노멀이 옛 값(첫 정점). **수정**:
    `recomputeNormals` — 클러스터된 실제 면으로 면적가중 스무스 노멀 재계산 → 지형 음영 복원.
- 색(지형이 흰색/초록?)은 재질 문제로 별개 — 로그 확인 후 판단(현 우선순위 아님).
...
### ✅ 한 일 (이번 세션)
- **곡선**: SPLINE 정식 B-스플라인(de Boor), ELLIPSE 파라메트릭, ARC 0.4% 허용오차 테셀레이션.
- **흰선**: 초록 등고선(F0017111 #009800) 위 잔재는 순백이 아니라 **중간회색(#808080) 표고/지반
  레이어(ELEV-6·EZ-BASE)**. `isGrayish + surfaceLayerRe(elev·ez-base·contour·등고…)` 로 숨김.
- **선종류(대시)**: LTYPE 패턴 → 실제 파선(직선·곡선 호길이 기준, 위상연속). 점(0) 요소 무한루프
  버그를 최소길이 바닥처리로 방지(격리테스트 통과).
- **문자(한글 포함)**: `opentype.js` + 러너 설치 폰트(NanumGothic.ttf)로 TEXT/MTEXT 아웃라인을
  선분화. **charToGlyph 글자별 배치**로 GSUB shaping 크래시 회피(중요: getPath(문자열)은 크래시남).
  .ttc 는 opentype.parse 미지원 → .ttf/.otf 만. 폰트 없으면 라틴 내장폰트 폴백.
- **Z 진단**: 레이어별 표고범위 `Z[min~max] [평탄]` 로그 추가.

### ✅ 한 일 (2차 — 진짜 데이터로 방향 전환 + 선택 하이라이트)
- **DWG 3D 소스를 SVF 로 전환(핵심)**: 사용자 요구 = Civil3D **진짜 지표면(TIN)·코리더** 를 읽어라
  (Fuzor 스샷). 이 AEC 객체는 **오토데스크 SVF 변환만** 3D 메시로 준다. LibreDWG DXF 는 못 내보내
  (표면·코리더 유실 + 폭발 잔재 선 = '왜 불필요한 선/왜 안보임'의 근본원인). → `api/aps-convert.ts`
  DWG 기본 경로를 **SVF**(`convert4d.mjs`)로, DXF 는 `body.dxf===true` 옵트인만.
- **직접 만든 TIN(Delaunay) 전면 제거**: 사용자 "이런건 만들지마"(지오메트리 날조 금지). delaunator 제거.
- **뷰어 선택 하이라이트**: 클릭 객체 형광 강조(highlightMaterial glowThrough)+이전 해제, 노드명 표시.
- (유지) 문자 한글(opentype)·선종류 대시·레이어별 Z 진단 — DXF 옵트인 경로에서 유효.

### 🧱 NWD 지표면 확인 + 통합모델 대용량 감량 (2026-08-11)
- **NWD 에 진짜 3D 지표면 있음(확인)**: 실패 로그(08-10)에 최대메시 `82,403,103정점 size=(7090,6018,108.1)`
  — Z높이 **108m**(DWG 의 평면 Z=0.3 과 대조). 소스는 NWD 가 맞다.
- **문제**: 통합모델 NWD = **삼각형 2억5천만·메시 49만**. GLB 조립 때 메모리 초과로 죽음(NWD 변환
  08-06·08-10 **둘 다 exit143**). 6GB 힙으로 못 담음.
- **해결(구현)**: `mergeGlb` 에 **전역 삼각형 예산**(`MERGE_TRI_BUDGET` 기본 1천만) — 1패스로 총
  삼각형 세고 예산 맞춰 전 프래그먼트 균등 감량(globalRatio). 256M→~10M. 예산 이하 모델은 영향 없음.
  워크플로 힙 6GB→12GB(러너 16GB). 지표면 고저 유지하며 로드 가능.
- **2차 수정(a91c54e 실패 원인)**: 예산 감량을 넣었는데도 256M→104M 밖에 안 줄어 GLB 버퍼 4.55GB
  → uint32(4GB) 초과로 죽음. 원인: 프래그먼트 49만 개가 각 ~500삼각형인데 `LockBorder`로 경계를
  잠그니 거의 안 줄음(정점 대부분이 경계). → **대용량 모드는 LockBorder 해제**(globalRatio<1) +
  예산 8M. 목표 ~8M삼각형·버퍼 ~0.25GB. 이음매 미세 틈 감수(로드 우선).
- 재변환 후 로그 `총메시삼각형 … 적용비 … 경계잠금 false` + `삼각형 총 ~8M` 확인.

### 🧭 DWG=DXF(깔끔) / 지표면=NWD 확정 (2026-07-31)
- **결정적 확인**: ACC 자체 3D 뷰에서도 이 DWG 는 **지표면이 안 보인다**(사용자 스샷). 즉 DWG
  자체에 음영 지표면 없음(형상은 등고선에만). SVF 실행 로그도 동일 증명 — 지표면 후보 메시가
  `size=(4857,3723,0.3)` 완전 평면, 반면 갱구 구조물 메시는 `Z=31.6` 정상(→ 우리 파이프라인 정상).
- **NWD 는 보인다**(사용자): Navisworks 가 지표면을 **메시로 구워** 저장하기 때문. → **진짜 음영
  지표면 소스 = NWD**. 앱은 nwd 를 이미 SVF 경로로 처리(코드 수정 불필요) — NWD 를 ACC 에서 선택.
- **DWG 라우팅 원복**: `aps-convert` `useDxf = isDwg && project && item` — DWG 는 다시 **DXF 경로**
  (깔끔한 벡터 선형·문자). SVF 는 DWG 에 지표면 이득 없이 무지개색·평면 시트만 더해 더 지저분(개판)
  했음. NWD/기타(isDwg=false) → SVF 유지.
- 다음: 사용자에게 NWD 변환 요청 → 로그로 지표면 메시(높이 있음) 확인 → 색·정합 다듬기.
...
- **평탄 Z**: 뷰어 상태바 `AABB span(7855,0,6914)` 의 **가운데=표고=0** → 3D가 완전 평탄(스샷2 FRONT
  뷰 한 줄). 그런데 변환기 로그는 `Z 클램프 …→ [-125.9,186.0]`(312m 유지)로 **모순**. 두 가설:
  ① 등고선(F0017111)이 실제로 2D 평탄(Z=0)이고 312m는 소수 3D선(도로)만 → **지표면/코리더 3D는
  SVF 솔리드 병합 필요**(DXF 선만으론 불가). ② 뷰어가 옛 GLB 로드(프리사인드라 캐시 아님이 유력).
  → **이번에 넣은 `레이어별 Z[min~max]` 로그가 F0017111 평탄 여부를 확정**. 재변환 로그 먼저 확인 후 분기.
- **지표면·코리더 3D**: 사용자 요구. DXF 는 평탄 선. 진짜 3D는 SVF 경로(convert4d.mjs)+`mergeGlb.mjs`
  병합이 정공법. Z 진단 결과 보고 착수.

### 🔜 다음 할 일
1. 재변환 로그의 `그려진 레이어 상위 … Z[min~max]` 로 F0017111 평탄 여부 확정 → Z 전략 결정.
2. 평탄이면 SVF 3D(지표면 TIN·코리더 솔리드) 병합 파이프라인 착수.
3. 한글 문자 실렌더 눈확인(NanumGothic 로드 로그 `문자 폰트 로드:` 확인).
...
### ✅ 한 일 (8차 — 토목 DWG 네비게이션·렌더 격파: 사용자 라이브 피드백 반복)
- **줌 안 됨**: xeokit 기본 휠은 절대속도라 측량좌표(225km)에선 거의 안 움직임 → 내장 휠
  끄고(`mouseWheelDollyRate=0`) **비율 기반 커스텀 휠**(커서 지점/look 향해 18%씩) + ＋/－ 버튼.
- **좌드래그 회전 안 됨**: CameraUpdater 는 orbit 회전을 `followPointer && pivoting` 일 때만
  적용(fallback 없음). 커스텀 휠 때문에 followPointer=false 로 뒀던 게 회전을 죽였음 →
  `followPointer=true`(휠은 rate=0 으로 차단) + `smartPivot=false`(20km 가상구 대신 look 기준).
- **더블클릭=전체맞춤**: 병합 메시라 엔티티=전체 → 엔티티 AABB 대신 **클릭 표면 지점**으로
  flyTo(`flyToWorldPoint`). '선택 확대' 버튼도 지점 기반.
- **NavCube**(방향 큐브) 추가, 배경 토글(흰/어둠).
- **CAD 내용 대부분 안 보임 — 핵심 3연타(로그로 GLB 내부 정점단위 확진)**:
  1) 색상7(순수 흰색) 선을 **검정으로**(흰 배경 가독성, CAD 흰종이 플롯과 동일). 유채색 보존.
  2) 모델 전체(19.5km) 덮는 **거대 흰색 채움면(정점 15.9만)이 뒤 선형을 가림** → DWG 흰색
     솔리드 렌더 제외. 검정 솔리드(코리더)→회색.
  3) **xeokit line 프리미티브 정점 65,535 한계** — 대형 선 그룹(지형 삼각망 54만·46만 정점)이
     통째로 렌더 실패. → **선 그룹을 6만 정점 버킷으로 분할**(mesh 163→183). + DWG 는 Draco
     끔(대형 line 손상 방지, R2 는 크기 무제한, 43MB 비압축).
- 진단 로그 상시화: `[diag]`(노드종류·삼각형/선분 총량·최대메시), `[grp]`(그룹별 색·정점·크기).
- **미확정**: 위 3연타 후 지형 삼각망이 실제로 보이는지 사용자 확인 대기(마지막 재변환 완료).
  안 보이면 남은 후보 = 유채색 대형 채움면 가림 / xeokit 대형 line 추가 한계.

### 인수인계 한 줄 (8차)
토목 DWG 렌더 3대 버그(색상7 흰선·거대 흰 채움면 가림·line 65535 정점한계) 격파 + 네비(비율휠·
회전 followPointer·지점 더블클릭). 선 그룹 6만 버킷 분할 + DWG Draco off. 확인 대기 중.
