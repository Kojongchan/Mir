...
---
## 📋 [기획] 방향 전환 — 메뉴별 고도화 로드맵 (2026-07-05)
> branch `claude/magical-curie-d4relv`(기획, 코드 변경 없음). main 최신(U1~U4·Q1·M1) 병합해 동기화.
- **방향**: 신규 기능 남발 중단 → **좌측 메뉴 14개를 하나씩 고도화**해 실사용 완성도 상향. → PLANNING **§0-K(M-시리즈)**.
- 앞서 정의한 U(디자인)·R(기능, `docs/handoff/R-series.md`)·S(뷰어) 항목을 **메뉴 단위로 재배치**한 실행표 작성.
- **권장 순서**: 게시판(→회의록 R2)·협업/이슈(대시보드+RFI/Punch)·자료관리(ISO19650)·공사일보(모바일)·물량/기성
  (원가·EVM)·통합모델(V2/V3)·공정현황(EVM)·구성원권한(역할·관계사)·사업개요(실시간) …
- 신규 대형(Track C·R-AI)은 고도화 사이클 이후/예산·데이터 결정 시. **인수인계 한 줄**: §0-K 순서대로 메뉴 고도화 착수.
