...
**마지막 업데이트**: 2026-06-22 · **S45 뷰어 추가요청 1·2·3 반영**(원형 단면·속성 그룹팝업·객체 트리·선택 연동).
- **추가1 원형 단면**(#66): `CIRCLE_SEGMENTS` 12→24 — 원기둥이 매끈한 원으로.
- **추가3 속성정보 팝업**(#66): IFC 속성세트/수량세트를 **세트명별 그룹**으로 묶어 접기·크기조절 팝업(툴바 ℹ 속성정보, 선택 시 자동).
- **추가2 객체 트리 + 추가3 양방향 연동**(#67): 사이드바 객체 트리(카테고리 묶음·눈 토글), 객체↔3D 선택 연동(자동 펼침·스크롤),
  사이드바 좌우 + 모델폴더/객체 상하 리사이즈.
**남은 추가요청**: **추가4**(툴바 카테고리 그룹화) · **B-2**(버전 V1/V2 전환·중첩 UI, 열린 상태 자동갱신).
**운영 필수**: 0019 마이그레이션 DB 적용(B-1 연동 전제). **문제4**: BIM 파일 삭제 시 연동 models 자동 제거, 레거시분은 대시보드 정리.
