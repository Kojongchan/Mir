...
### ✅ 한 일 (사용자 결정 반영)
- **게시판**: 유지(결정1).
- **협업 탭 병합**(결정2): 이슈·RFI·Punch 를 사이드바 1개 "협업 (이슈·RFI·Punch)"로 통합. 각 페이지 상단에
  `ModuleTabs`(공통 탭바) 추가 — 라우트(/issues·/rfi·/punch)는 유지해 딥링크·검색 이동 보존.
- **원가·기성 탭 병합**(결정3): 기성내역·EVM 을 "원가 · 기성" 1개로. Billing·Evm 상단에 costTabs 탭바. EVM
  사이드바 제거(/evm 라우트 유지). 기성내역 라벨 → "원가 · 기성".
- **기상알림 위젯화**(결정4): 사이드바 '기상알림' 제거. **MIR Board 위젯 'weather_alerts'** 신설(활성 규칙·오늘
  경보 수 + "지금 점검" + "규칙 관리 →" 링크). /weather 라우트·관리 페이지는 위젯 링크로 접근 유지.
- **원격지원 삭제**(결정5): 사이드바·라우트·`RemoteSupport.tsx`·`lib/remote.ts` 제거. 마이그 0040·DB 는 보존(비파괴).
- 공통 컴포넌트: `components/ModuleTabs.tsx`(collabTabs/costTabs 헬퍼) + `.mod-tabs` CSS.

### 🔜 다음 할 일 / 미해결
- 라이브 확인: 협업/원가 탭 전환·MIR Board 기상 위젯 추가·"지금 점검". 마이그(0036 재실행·0042) 여전히 선행.
- 후속 통합 후보(미결): 회의록↔게시판 통합(현재 게시판 유지), 자재·자산/제출물 등은 유지.

### 인수인계 한 줄
메뉴 정리 완료(협업 3종·원가 2종 탭 병합, 기상 위젯화, 원격지원 삭제 → 23→18). 라우트는 유지해 딥링크 보존.

---
## 📋 라이브 피드백 수정 5종 (문제1~5) (2026-07-03)
> branch `claude/rfi-management-system-2bw5lp`. typecheck·build 통과. 마이그 0042 추가.

### ✅ 한 일
1. **문제1 — 제출물 연결파일**: 옛 시스템/ACC 미러 파일(*.ifc/*.nwd)이 뜨던 것 → `listCdeDocs`(source=supabase,
   폴더 그룹) 신설로 **자료관리 문서만 폴더별 optgroup** 표시.
2. **문제2 — EVM 미작동**: 실패 원인을 화면에 표면화(`Promise.allSettled`+에러 메시지). cost_rates/evm_actual 없으면
   "수정된 0036 재실행" 안내를 페이지에 노출 → 원인 진단 가능. (근본 해결은 0036 재실행.)
3. **문제3 — 페이지 구분**: `PageHeader`(도메인 아이콘 + 부제) 신설, RFI/Punch/회의록/제출물/EVM/기상/자재/원격
   8개 페이지에 적용 → 리스트형 페이지들이 아이콘·부제로 구분됨.
4. **문제4 — 개명**: HIBoard → **MIR Board**(nav·h1·검색 팔레트). (현대 'HI' 오해 방지, 쌍용.)
5. **문제5 — 관계사 통합**: 별도 '관계사' 메뉴 삭제 → **구성원·권한 화면에 통합**(`CompaniesPanel`, 관리자 CRUD).
   `0042` 로 project_members·profiles 를 같은 프로젝트 멤버가 열람 가능하게 확장(SECURITY DEFINER `shares_project`
   로 재귀 회피) → **실무자·뷰어는 구성원 로스터(이름·소속·역할·접근권한)를 열람 전용**으로 봄. 구성원·권한 메뉴는
   전체 멤버에게 노출(비관리자는 로스터만).

### 🔜 다음 할 일 / 미해결
- **마이그 0042 적용** 후 로스터 열람 확인. EVM 은 **수정된 0036 재실행** 필수.
- 메뉴 통합 추천(사용자 진행 예정) — 아래 별도 섹션 참고.

### 인수인계 한 줄
라이브 피드백 5종 수정 완료(제출물 CDE 목록·EVM 오류표면화·PageHeader 구분·MIR Board 개명·관계사→구성원 통합+0042
로스터). 남은 건 0036 재실행·0042 적용·메뉴 통합(사용자).

### 📌 메뉴 통합 추천(사용자 결정용)
현재 23개 → 권장 정리:
- **협업 묶기**: 이슈·RFI·Punch 는 성격이 유사 → 상위 "협업" 탭 그룹 또는 이슈에 RFI/Punch 탭 병합 검토.
- **게시판 삭제**: R2 회의록이 게시판을 대체하도록 기획됨 → 게시판 제거 후보 1순위.
- **기상알림**: 전용 메뉴보다 사업개요/MIR Board 위젯으로 흡수 검토.
- **원격지원**: 사용빈도 낮음 → 자료관리 하위 또는 후속 보류 후보.
- **EVM ↔ 기성내역**: 원가 성격 유사 → "원가·기성" 한 메뉴에 탭 통합 검토.
- (완료) 관계사 → 구성원·권한 통합, 하도급내역 삭제.
