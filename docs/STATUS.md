...
### ✅ 한 일 (2차 — 지표면 복원·선택 하이라이트)
- **지표면(TIN) 복원**: LibreDWG 가 Civil3D 표면을 면으로 안 내보냄(3DFACE=0) → 등고선·브레이크
  라인 **선의 3D 정점을 15m 격자로 모아 Delaunay 삼각망** 생성(회색 음영면, mode:4+법선). 원본
  셰이드 뷰처럼. 표고 평탄하면 자동 생략+로그. `delaunator` 의존성. 노드명 `지표면(TIN)`.
- **흰선 숨김 과도 수정(중요)**: `등고/contour/tin` 일반어가 진짜 등고선·식재(PLAN**TIN**G!)까지
  숨겨 '다 안 보임' 유발 → **확인된 회색 잔재(ELEV-*·EZ-BASE·지표면/surface)만** 좁게 숨김.
- **뷰어 선택 하이라이트**: 클릭 객체 형광 강조(highlightMaterial glowThrough) + 이전 해제.
  노드명(지표면(TIN) 등) 속성패널 표시. `ThreeDTest.tsx`.
